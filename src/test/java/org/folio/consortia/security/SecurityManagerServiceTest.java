...
class SecurityManagerServiceTest extends BaseTest {
  ...
  void prepareSystemUser() {
    ...
    wireMockServer.stubFor(get(urlEqualTo("/users?query=username%3D%3Dmod-consortia")).willReturn(
      aResponse().withHeader("Content-Type", MediaType.APPLICATION_JSON_VALUE)
        .withBody(SYS_USER_EXIST_RESPONSE)));

    wireMockServer.stubFor(get(urlEqualTo("/perms/users?query=userId%3D%3Da85c45b7-d427-4122-8532-5570219c5e59")).willReturn(
      aResponse().withHeader("Content-Type", MediaType.APPLICATION_JSON_VALUE)
    ...
    wireMockServer.verify(getRequestedFor(urlEqualTo("/users?query=username%3D%3Dmod-consortia")));
    wireMockServer.verify(putRequestedFor(urlEqualTo("/users/a85c45b7-d427-4122-8532-5570219c5e59")));
  }
}
